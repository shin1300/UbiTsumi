...
## 概要
- プレイ: `Assets/Scenes/Home.unity` から再生（仮想ジョイスティック対応）
- ゲーム: 動物/オブジェクトを積み上げて安定させる
- カスタムモデル: GLB 読み込みに対応（glTFast + SimpleFileBrowser）

## デモ
- 動画: docs/movie/demo.mp4
...
## 命名規則とディレクトリ構成（ガイド）
- Scenes: `Home.unity`（起動用）/ `GameScene.unity`（本編）
- Scripts: クラス/ファイル名はパスカルケース（例: `GameManager`, `DataPersistence`）。用途別にサブフォルダ可。先頭に1行の概要コメントを付与
- Animals: 軽量サンプルのみ常備。大容量は Git LFS 管理（`.gitattributes` 済）や任意導入に留める
